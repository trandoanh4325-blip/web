<?php
session_start();
// Gọi file kết nối CSDL (Lưu ý: KHÔNG dùng hàm ensure_logged_in() ở đây)
...
// Nếu nhập sai mã ID trên URL -> Báo lỗi
if (!$product) {
...
<head>
  <meta charset="UTF-8">
  <title><?= h($product['ten_sp']) ?> - Florentino Shop</title>
...
<body>
  ...
  <div class="product-detail">
    ...
    <div class="info-section">
      ...
      <div class="option">
        <label>Phân loại:</label> <?= h($product['ten_loai']) ?>
      </div>
      <div class="option">
        <label>Mã SP:</label> <?= h($product['ma_sp']) ?>
      </div>
      <div class="option">
        <label>Số lượng tồn:</label> <?= (int)$product['so_luong_ton'] ?>
      </div>

      <div class="option">
        <label>Số lượng mua:</label>
        <input type="number" id="quantity" value="1" min="1" max="<?= (int)$product['so_luong_ton'] ?>">
      </div>

      <!-- KHÓA NÚT MUA VÀ THÊM GIỎ HÀNG BẰNG REQUIRE-LOGIN -->
      <div class="buttons">
        <a href="#" class="require-login"><button class="add-cart">Thêm vào giỏ hàng</button></a>
        <a href="#" class="require-login"><button class="buy-now">Mua ngay</button></a>
      </div>
  ...
  <div class="product-description">
    ...
    <details class="desc-box">
      ...
      <div class="desc-preview">
        <p><?= nl2br(h((string)($product['mo_ta'] ?: 'Sản phẩm được làm thủ công tỉ mỉ, mang phong cách thanh lịch, phù hợp làm quà tặng cho người thân và bạn bè.'))) ?></p>
      </div>
    </details>
  </div>
  ...
  <div class="footer-wrapper">
    ...
    <!-- Gọi JS của Popup để xử lý class require-login -->
    <script src="js/popup.js"></script>
  </div>

<!-- POPUP BẮT BUỘC ĐĂNG NHẬP -->
